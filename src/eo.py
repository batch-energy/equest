...
class Space(Object):
    ...
    def testSpace(self,
        ...
        space_z = self.global_z()
        zone_type = self.zone_type()

        tests = [
            # O-kinds test for any intersection
             xMinO=='-' or (x_max > xMinO),
             yMinO=='-' or (y_max > yMinO),
             xMaxO=='-' or (x_min < xMaxO),
             yMaxO=='-' or (y_min < yMaxO),
        
            # I kinds check for complete containment
             xMinI=='-' or (x_min > xMinI),
             yMinI=='-' or (y_min > yMinI),
             xMaxI=='-' or (x_max < xMaxI),
             yMaxI=='-' or (y_max < yMaxI),
            
             zMin=='-' or (spaceZ < zMin),
             zMax=='-' or (spaceZ > zMax),
             zThru=='-' or
                (space_z < zThru) and ((spaceZ + space_height) > zThru),

             hMin=='-' or (space_height > hMin),
             hMax=='-' or (space_height < hMax),
 
            isPlenum.lower() == '-' or
            (isPlenum.lower() == 'y' and zone_type == 'PLENUM') or 
            (isPlenum.lower() == 'n' and zone_type != 'PLENUM'), 

            isConditioned.lower() == '-' or
            (isConditioned.lower() == 'y' and zone_type == 'CONDITIONED') or 
            (isConditioned.lower() == 'n' and zone_type != 'CONDITIONED'), 

            isUnconditioned.lower() == '-' or
            (isUnconditioned.lower() == 'y' and zone_type == 'UNCONDITIONED') or 
            (isUnconditioned.lower() == 'n' and zone_type != 'UNCONDITIONED'), 
            
            regex=='-' or re.search(regex, self.name)]
