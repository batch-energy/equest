...
def convert_feet(s):
    feet = float(s.split('_')[0])
    if '_' in s:
        feet += float(s.split('_')[1])/12
    return feet
...
class Svg_Origin():
    ...
    def __init__(self, svg_rect):
        ...
        self.svg_rect = svg_rect
        self.wall = svg_rect.title.split()[1]
